...
pub fn compile(stack_frame: &StackFrame, term: &Term) -> Result<Function, Error> {
    let mut memory_map = Box::new(
        ExecutableAnonymousMemoryMap::new(4096, false, false)
    ...
    compile_term(stack_frame, &mut stream, Register64Bit::RAX, term).map_err(Error::CompileError)?;
...
fn compile_term(
    ...
    match term {
        Term::Expression(operator, args) => compile_expression(
            stack_frame,
            stream,
            destination,
            operator,
        ...
        Term::Definition(_definition) => compile_literal(stream, destination, &Literal::Str("<function>".to_owned()))
    }
}
    ...
    if let Operator::CallFunction(identifier) = operator {
        match stack_frame.resolve(&identifier) {
            ...
            Some(Symbol::Argument(_)) => {
                return Err(CompileError::NotImplemented("calling function arguments".to_owned()));
            },
            None => {
...
fn compile_term_argument(stream: &mut InstructionStream, stack_frame: &StackFrame, destination: Register64Bit, term: &Term) -> Result<(), CompileError> {
    ...
    match term {
        ...
        Term::Identifier(identifier) => { 
            match stack_frame.resolve(&identifier) {
                ...
                Some(Symbol::Function(_function, _arity)) => {
                    return Err(CompileError::NotImplemented("functions as expression arguments".to_owned()));
                },
                None => {
                    return Err(CompileError::UnresolvedSymbol(identifier.clone()));
                }
        ...
        Term::Definition(_definition) => { return Err(CompileError::NotImplemented("function definition as function argument".to_owned())) },
    }
...
fn parameter_register(index: &usize) -> Result<Register64Bit, CompileError> {
    match index {
        ...
        _ => {
            Err(CompileError::NotImplemented("functions with arity greater than 6".to_owned()))
        }
    }
}